...
public class LeaveoffCardForm  extends TBMPubBillCardForm implements BillCardBeforeEditListener{
	...
	public void initUI() {
		...
		diffPanel.getUITextField().setTextFiledFormatterFactory(new DefaultTextFiledFormatterFactory(new LeaveoffDiffNumFormatter(diffPanel.getUITextField())));
	}
	...
	protected void setDefaultValue() {
		...
		//和自助保持一致，默认时间为登记时间
		// 本地化需求，销假默认全部销假 Modified by Ethan Wu 2019-06-17 start
...
//		getBillCardPanel().getHeadItem(LeaveoffVO.LEAVEENDTIME).setValue(regvo.getLeaveendtime());
		...
		// 本地化需求，销假默认全部销假 Modified by Ethan Wu 2019-06-17 end
	...
	public void rollBackCode(String autoGeneratedCode){
		if(StringUtils.isNotEmpty(autoGeneratedCode)){
			...
			try {
				hrBillCode.rollbackPreBillCode(LeaveConst.BillTYPE_LEAVEOFF, getModel().getContext().getPk_group(),  getModel().getContext().getPk_org(), autoGeneratedCode);
	...
	protected ChangeLabelVO getChangeLabelVO() {
		if(changeLabelVO == null){
			...
			changeLabelVO.setChangeLabelKeys( new String[]{
					"regleavehourcopy","pk_leavereg.resteddayorhour","pk_leavereg.realdayorhour","pk_leavereg.restdayorhour",
					"pk_leavereg.freezedayorhour","pk_leavereg.usefuldayorhour","pk_leavereg.lactationhour"
					});
		}
	...
	protected String[] getFloatItems() {
		return new String[]{"pk_leavereg.leavehour","pk_leavereg.resteddayorhour","pk_leavereg.realdayorhour","pk_leavereg.restdayorhour",
				"pk_leavereg.freezedayorhour","pk_leavereg.usefuldayorhour","pk_leavereg.lactationhour",LeaveoffVO.REALLYLEAVEHOUR,LeaveoffVO.DIFFERENCEHOUR,LeaveoffVO.REGLEAVEHOURCOPY};
	}
	@Override
	...
	public void afterEdit(BillEditEvent e) {
		...
		if(LeaveoffVO.TRANSTYPEID.equals(e.getKey())) {
			...
			//同时记录相应修改字段的日期
			LeaveoffVO leaveoffvo = (LeaveoffVO)getBillCardPanel().getBillData().getHeaderValueVO(LeaveoffVO.class.getName());
			if(leaveoffvo==null)return;
...
//				getBillCardPanel().setHeadItem(LeaveoffVO.LEAVEOFFBEGINDATE,leaveoffvo.getLeaveoffbegintime()==null?null:leaveoffvo.getLeaveoffbegintime());
				getBillCardPanel().setHeadItem(LeaveoffVO.LEAVEBEGINDATE,leaveoffvo.getLeavebegintime()==null?null:leaveoffvo.getLeavebegintime());
			}
			else {
//				getBillCardPanel().setHeadItem(LeaveoffVO.LEAVEOFFENDDATE,leaveoffvo.getLeaveoffendtime()==null?null:leaveoffvo.getLeaveoffendtime());
				getBillCardPanel().setHeadItem(LeaveoffVO.LEAVEENDDATE,leaveoffvo.getLeaveendtime()==null?null:leaveoffvo.getLeaveendtime());
			}
...
//				getBillCardPanel().setHeadItem(StringUtils.replace(e.getKey(), "date", "time"),leaveoffvo.getLeaveoffbegindate()==null?null:(new  UFDateTime(leaveoffvo.getLeaveoffbegindate().toDate().getTime())));
				getBillCardPanel().setHeadItem(StringUtils.replace(e.getKey(), "date", "time"),leaveoffvo.getLeavebegindate()==null?null:(new  UFDateTime(leaveoffvo.getLeavebegindate().toDate().getTime())));
			}
			else {
//				getBillCardPanel().setHeadItem(StringUtils.replace(e.getKey(), "date", "time"),leaveoffvo.getLeaveoffenddate()==null?null:(new  UFDateTime(leaveoffvo.getLeaveoffenddate().toDate().getTime())));
				getBillCardPanel().setHeadItem(StringUtils.replace(e.getKey(), "date", "time"),leaveoffvo.getLeaveenddate()==null?null:(new  UFDateTime(leaveoffvo.getLeaveenddate().toDate().getTime())));
			}
//			if(leaveoffvo.getLeaveoffbegindate()!=null && leaveoffvo.getLeaveoffenddate()!=null){
	...
	protected void onEdit() {
		...
		if(obj.equals(TimeItemCopyVO.LEAVETYPE_LACTATION))
			...
			islactation=false;
		changeTemplate();
		String pk_psndoc = (String)((UIRefPane)getBillCardPanel().getHeadItem(LeaveoffVO.PK_PSNDOC).getComponent()).getRefPK();
...
//			billCardPanel.getHeadItem("pk_leavereg.leaveenddate").setShow(islactation);
	...
	public void setEditable(boolean editable) {
		...
		if (getModel().getUiState() == UIState.ADD || getModel().getUiState() == UIState.EDIT){
			setHeadItemEnable(LeaveoffVO.BILL_CODE, ((LeaveoffAppModel)getModel()).isBillCodeEditable());
		}
	}
